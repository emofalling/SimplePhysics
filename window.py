from PyQt5.QtWidgets import (
    ...
    QComboBox, QSlider, QCheckBox, QSpinBox, QDoubleSpinBox,
    # 提示枚举类
    QSizePolicy,
    # 图形
...
class WinMain(QMainWindow):
    ...
    def initcontent(self):
        ...
        self.control_panel_layout.addLayout(self.scale_layout)
        self.scale_label = QLabel("倍率")
        self.scale_layout.addWidget(self.scale_label)
        self.scale_label.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        self.scale_slider = QDoubleSpinBox()
        self.scale_slider.setMaximum(1e+10)
        ...
        # 延迟补偿
        self.delay_compensation_checkbox = QCheckBox("延迟补偿(物理模拟异常时关闭)")
        self.control_panel_layout.addWidget(self.delay_compensation_checkbox)
        # 默认打开
        self.delay_compensation_checkbox.setChecked(True)
        # 在发生碰撞时暂停
        self.collision_pause_checkbox = QCheckBox("发生碰撞时暂停")
        self.control_panel_layout.addWidget(self.collision_pause_checkbox)
        # 默认打开
        self.collision_pause_checkbox.setChecked(False)
        
    def __map_pos(self, x, y) -> tuple[float, float]:
        # 将物理坐标映射到屏幕坐标
        return (
            x*ui_scale,
            self.graphics_scene.height() - y*ui_scale
        )
    ...
    def draw_scene(self):
        ...
        for obj in phys._fixed_objects+phys._active_objects:
            if isinstance(obj, Line):
                ...
                start, end = obj.intpos_in_rect(self.graphics_view.width()/ui_scale, self.graphics_view.height()/ui_scale)
    ...
    def draw_force_analysis(self):
        ...
        for obj in phys._active_objects:
            # 如果没有速度，不绘制
            if obj.velocity.abs_square() < DIV_EPLISON:
                continue
            if isinstance(obj, Circle):
                # 绘制运动方向箭头
                start = obj.pos
                end = obj.pos + obj.velocity * speed_scale
    ...
    def draw_collision_analysis(self, events: list[CollisionEvent]):
        for event in events:
            ...
            # 绘制各物体的碰撞前速度向量
            for obj, v in (
                (event.obj1, event.obj1_last_velocity),
                (event.obj2, event.obj2_last_velocity)
            ):
                if v.abs_square() < DIV_EPLISON:
                    continue
                start = obj.pos
                end = obj.pos + v * speed_scale
                line = QGraphicsLineItem(*self.__map_pos(start.x, start.y), *self.__map_pos(end.x, end.y))
                ...
                arrow_item = QGraphicsPolygonItem(polygon)
                arrow_item.setBrush(QBrush(QColor(0, 100, 0)))
                arrow_item.setPen(pen)
                self.graphics_scene.addItem(arrow_item)
                self.collision_analysis_cache.append(arrow_item)



        # 若有碰撞点，且开关被打开，则暂停
        if len(events) > 0 and self.collision_pause_checkbox.isChecked():
            self.phys_stop()
            pass
    ...
    def initscene(self):
        ...
        for obj in phys._fixed_objects+phys._active_objects:
            if isinstance(obj, Line):
                # 计算点向式直线与x轴和y轴的交点
                start, end = obj.intpos_in_rect(self.graphics_view.width()/ui_scale, self.graphics_view.height()/ui_scale)
                print(obj.name, self.graphics_view.width()/ui_scale, self.graphics_view.height()/ui_scale, "->", start, end)
                if start is None or end is None:
                    start = (-1, 1)
                    end = (-1, 1)
                # 添加一条线
                ...
                # 添加一个圆
                radius = obj.radius
                circle = QGraphicsEllipseItem(-radius*ui_scale, -radius*ui_scale, radius*2*ui_scale, radius*2*ui_scale)
    ...
    def single_step(self):
        # 仅模拟一步
        if self.delay_compensation_checkbox.isChecked():
            phys.update(self.actual_delay * self.scale_slider.value())
        else:
            phys.update(tick * self.scale_slider.value() / 1000)
        # 更新场景
        self.draw_scene()
        # 如果开启碰撞分析，绘制碰撞分析
        if self.collision_analysis:
